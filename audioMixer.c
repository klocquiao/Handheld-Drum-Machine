...
#define VOLUME_INCREMENT_DECREMENT_VALUE 5
...
void AudioMixer_queueSound(wavedata_t *pSound)
{
	...
	if (isSpaceFound == false) {
		printf("Failed to play the given wave file!\n");
	}
}
...
void AudioMixer_incrementVolume() {
	AudioMixer_setVolume(AudioMixer_getVolume() + VOLUME_INCREMENT_DECREMENT_VALUE);
}
...
static void fillPlaybackBuffer(short *playbackBuffer, int size)
{
	...
	pthread_mutex_lock(&audioMutex);
	{
		for (int i = 0; i < MAX_SOUND_BITES; i++) {
			if (soundBites[i].pSound != NULL) {
				bool isToBeComplete = false;
				int offset = soundBites[i].location;
				int remainder = soundBites[i].pSound->numSamples - offset;
				int loopSize;

				if (remainder <= playbackBufferSize) {
					loopSize = remainder;
					isToBeComplete = true;
				}
				else {
					loopSize = playbackBufferSize;
				}

				for (int j = 0; j < loopSize; j++) {
					int temp = (int) playbackBuffer[j] + soundBites[i].pSound->pData[offset];
					if (temp < SHRT_MIN) {
						...
						playbackBuffer[j] += soundBites[i].pSound->pData[offset];
					}
